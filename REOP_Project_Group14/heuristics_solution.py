...
def check_solution(solution: List[Tuple[Job, Task, Machine, Operator]]):
    for job, task, machine, operator in solution:
        ...
        if machine not in task.machines:
            return False
        if operator not in machine.operators:
            return False
    return True
...
def cost(solution: List[Tuple[Job, Task, Machine, Operator]]):
    ...
    time_per_job = {j: 0 for j in jobs_list}
    for job, task, machine, operator in solution:
        wait_time = max(time_per_machine[machine], time_per_operator[operator])
        time_per_job[job] += max(wait_time - time_per_job[job], 0) + task.processing_time
        time_per_machine[machine] += task.processing_time
...
def get_initial_solution():
    ...
    for j in jobs_list:
        for t in j.sequence_tasks:
            ...
            o = np.random.choice(list(m.operators))
            s.append((j, t, m, o))
    return s
...
def pertubate_solution(solution):
    ...
    new_o = np.random.choice(list(new_m.operators))
